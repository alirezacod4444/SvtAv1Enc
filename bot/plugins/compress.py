...
from bot.database import getffmpeg1

from .devtools import progress_for_pyrogram, progress_for_pyrogram1
from .extras import upload_handle1
...
async def sysinfo(e):
    ...
    while time.monotonic() - start_time <= 90:
        ...
        cpu_bar = [
            "⬢" * int(percent / 10) + "⬡" * (10 - int(percent / 10))
...
async def stats(bot: Client, event: CallbackQuery):
    try:
        ...
        await event.answer(
            "✖️ Error 404: File | Info not Found,\nKindly Resend Media.",
            show_alert=True,
        )
        LOGS.info(e)
...
async def parser(name):
    ...
    if "anime_season" in dic.keys():
        ...
        string = string + f"_S{season}"
    else:
        anime = ""
    # string = string + " [@XtasyFlicks]"
    return string
...
async def mediainfo(app, message):
    if message.reply_to_message:
        ...
        filepath = await app.download_media(
            ...
            file_name=Config.DOWNLOAD_DIR,
            progress=progress_for_pyrogram,
            progress_args=(app, "📥 **Downloading Video:**", msg, d_start),
        )
        ...
        await app.send_message(
            message.chat.id,
            "⚠️ **Reply To A File To Download It.**",
